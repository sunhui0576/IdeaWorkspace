package com.atguigu.flink.apitest.tabletest

import com.atguigu.flink.apitest.SensorReading
...
object FsOutputTest {
  def main(args: Array[String]): Unit = {
    ...
    // 2. 读取数据转换成流，map成样例类
    val filePath: String = "input/sensor.txt"
    ...
    // 5. 将结果表输出到文件中
    tableEnv.connect(new FileSystem().path("out/output.txt"))
//      .withFormat(new Csv())
