...
/**
  ...
  */
object Flink18_Transform_Operator1 {
...
//    val sumDS: DataStream[WaterSensor] = sensorDS.keyBy(_.id).sum(2)
    val minDS: DataStream[WaterSensor] = sensorDS.keyBy(_.id).sum(2)
//    val maxDS: DataStream[WaterSensor] = sensorDS.keyBy(_.id).max(2)
