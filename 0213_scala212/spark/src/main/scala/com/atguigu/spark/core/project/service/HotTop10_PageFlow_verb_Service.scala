...
class HotTop10_PageFlow_verb_Service extends ServiceCommon {
  ...
  override def analysis() = {
    ...
    //需要统计的页面
    val pageList = List(1,2,3,4,5,6,7,8)
    //需要统计的页面tuple：（1-2，1）
    val zipPage = pageList
                                    .zip(pageList.tail)
                                    .map(s => (s._1 + "-" + s._2))
    //todo 分母
    val pageCountMap = cacheRdd
                                      .filter(s=>pageList.contains(s.page_id.toInt))
                                      .map(s => (s.page_id, 1))
                                      .reduceByKey(_ + _)
                                      .collect()
                                      .toMap
                                      ...
                                      .mapValues(                               //对value的操作用mapValues
                                            iter => {
                                              ...
                                              val tupIds = ids
                                                                            ...
                                                                            .map(s => (s._1 + "-" + s._2, 1))
                                              tupIds.filter(s=>zipPage.contains(s._1))                   //返回 （1-2，1）
                                            }                                  // （session，（1-2，1））
                                        )
                                            // 过境只需要的统计的页面
                                      .map(_._2)                //不需要session
